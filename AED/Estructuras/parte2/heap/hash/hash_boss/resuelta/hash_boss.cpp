...
public:
	...
	{	
		MatrixElement* tmp = hash_table.find(MatrixElement(row,col,0));
	...
	void print()
	{
		...
		// Recorremos cada fila de la matriz me llego esta parte xd
		for (int r = 0; r < rows; ++r)
		{
			// Recorremos cada columna de la fila actual
			for (int c = 0; c < cols; ++c)
			{
				// Imprimimos el valor usando el get que ya programamos
				std::cout << get(r, c) << "    ";
			}
			// Al terminar una fila, damos un salto de línea
			std::cout << std::endl;
		}
