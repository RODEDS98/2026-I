...
public:
	...
	void pila_inorder();
	void height(CNode* n,int i ,int& max);
	int obtener_height();
...
void CBinTree::pila_inorder()              
{
	cout << "Pila inorder: " << endl ;
	CNode* n = root;
	pair<int, CNode*> par;
	stack< pair<int, CNode*> > p;
	...
	while(  !(p.empty()) ) {
		...
		if (p.top().first == 0) {
			p.top().first = 1;
			if (p.top().second->left) p.push(make_pair(0, p.top().second->left));
		}
		else if (p.top().first == 1) {
			p.top().first = 2;
			cout << p.top().second->v << " ";
		}
		else if (p.top().first == 2) {
			p.top().first = 3;
			if (p.top().second->right) p.push(make_pair(0, p.top().second->right));
		}
		else if (p.top().first == 3) {
			p.pop();
		}
...
void CBinTree::height(CNode* n,int i , int& max)
{
	if (!n) return;
	if(i >= max) max =i;
	height(n->left, i+1, max);
	height(n->right, i+1 , max);
}

int CBinTree::obtener_height()
{
	cout << endl << "Obtener la altura de otra forma" << endl;
	int max = 0;
	height(root,1,max);
	return max;
}
