...
void CAVLTree::balanceo(std::stack<CAVLNode**> & estructura){
    ...
    //int CAVLTree::MaxNodeHeight(CAVLNode* n)   ::   DEVUELVE EL tamaño
    ...
    for( ; !estructura.empty() ; estructura.pop() ){
        ...
        int alt_izq = MaxNodeHeight();
        int alt_der = MaxNodeHeight();

        // halla la altura maxima:
        int alt_izq=0;  
        int alt_der=0;  

    // SE RESTAN LAS ALTURAS DE EL NODO TOP() EN LA PILA MaxNodeHeight(CAVLNode* n)
