...
private:
    bool Find(int x, CAVLNode**& p, std::queue<CAVLNode*>& estructura);
...
void CAVLTree::balanceo() {

}

bool CAVLTree::Find(int x, CAVLNode**& p, std::queue<CAVLNode*>& estructura)
{
...
{
    std::queue<CAVLNode*> cola_nodos;
    CAVLNode** p;
    if (Find(x, p, cola_nodos)) return 0;
    // balanceo
    balanceo();
    *p = new CAVLNode(x);
